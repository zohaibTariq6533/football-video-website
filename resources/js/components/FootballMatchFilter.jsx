...
const FootballMatchFilter = ({
    ...
    // New states for player hover popup
    ...
    useEffect(() => {
        ...
        teams.forEach((team) => {
            initialSelectedEvents[team.id] = {};
            eventTypes.forEach((eventType) => {
                // Initially UNSELECT all events (changed from true to false)
    ...
    const togglePlayerSelection = useCallback((teamId, playerId, eventType) => {
        setSelectedPlayers((prev) => ({
            ...prev,
            [teamId]: {
                ...prev[teamId],
    ...
    // Apply filters - FIXED to show events without players
    useEffect(() => {
        const { analysisData } = initialData;
        ...
        <div className="w-full h-screen bg-gray-100 flex flex-col">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span 
                                                ...
                                                onMouseEnter={(e) => handleEventCountHover("no_team", eventType, e)}
                                    ...
                                    {allEventTypes.map((eventType) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span 
                                                    className="text-xs bg-gray-200 rounded-full px-2 py-1 cursor-pointer hover:bg-gray-300 transition-colors"
                                                    onMouseEnter={(e) => handleEventCountHover(team.id, eventType, e)}
                                                    onMouseLeave={handleEventCountLeave}
                                                >
                                                    {eventCount}
                                                </span>
                            ...
                            {filteredEvents.length > 0 ? (
                                filteredEvents
                                    ...
                                    .map((event, index) => {
                                        ...
                                        const hasSelectedPlayer = Object.values(selectedPlayers).some(
                                            (teamPlayers) =>
                                                teamPlayers &&
                                                Object.entries(teamPlayers).some(
                                                    ([playerId, selected]) =>
                                                        selected &&
                                                        (event.player_id === parseInt(playerId) ||
                                                         event.assistPlayerId === parseInt(playerId))
                                                )
                                        );
                                        ...
                                        return (
                                            <div
                                                key={event.id}
                                                className={`grid grid-cols-9 gap-2 p-3 border-b border-gray-200 transition-all duration-300 ${
                                                    index % 2 === 0
                                                        ? "bg-gray-50"
                                                        : "bg-white"
                                                } ${
            ...
            {hoveredEventCount && (
                <div 
                    ...
                    onMouseLeave={handleEventCountLeave}
                >
                    <h3 className="font-semibold mb-2 text-sm">Players for {hoveredEventCount.eventType}</h3>
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                        {getPlayersForEventType(hoveredEventCount.teamId, hoveredEventCount.eventType).map(player => (
                            <div key={player.id} className="flex items-center">
                                <input
                                    type="checkbox"
                                    id={`player-popup-${player.id}`}
                                    checked={selectedPlayers[player.teamId]?.[player.id] || false}
                                    onChange={() => togglePlayerSelection(player.teamId, player.id, hoveredEventCount.eventType)}
                                    className="mr-2 h-4 w-4 text-blue-600 rounded"
                                />
                                <label htmlFor={`player-popup-${player.id}`} className="text-sm">
                                    #{player.jerseyNo} {player.name}
                                </label>
                            </div>
                        ))}
                        {getPlayersForEventType(hoveredEventCount.teamId, hoveredEventCount.eventType).length === 0 && (
                            <p className="text-sm text-gray-500">No players in this event type</p>
                        )}
                    </div>
                </div>
            )}
